...
当前仓库已完成 Spec 002 的 Catalog、Spec 003/004/005/006/007/008/009 的 Workspace 与 Installation runtime，并完成 Spec 012 Control Plane Invocation Dispatch、Spec 013 A2A Router Foundation、Spec 014 Router-owned Invocation Ledger、Spec 015 Runtime B direct A2A sample、Spec 016 non-streaming dispatch、Spec 017 streaming A2A delivery、Spec 018 Invocation/Trace metadata reads、Spec 019 Agent SDK nested invocation、Spec 020 cross-runtime caller、Spec 021 Invoke-to-Record backend acceptance，以及 Spec 024 Router-to-Agent authentication；当前分支正在交付 Spec 025 Workspace Client SDK。已落地的运行时边界包括 Workspace owner policy、精确解析、Gateway v4 调用、Router-mediated JSON/SSE 调用、metadata-only Ledger、Workspace-scoped Invocation/Trace 查询、Workspace-bound Agent credentials、Runtime A -> Router -> Runtime B 嵌套调用、每请求 Ed25519 Router 凭证、Agent 侧严格验签与进程内一次性 `jti`，以及绑定单个 Gateway/Workspace/Owner credential 的应用 Client SDK。该 SDK 的每次调用仅接受 Agent ID、capability 与 JSON input，并严格校验 Result v1、Result Stream Event v2、Platform Error v4 和 Gateway-created Trace；它不接受 endpoint、Router、version、Release 或 Agent credential。Spec 021 与 Spec 024 的干净 Compose/PostgreSQL E2E 均已验收；Spec 024 的最终证据为 PR #55 CI run `30060752722`。Spec 025 已完成实现、完整 build/test/race/vet/lint、独立双轴 Review 与 `speckit-converge`，PR/CI/merge 证据仍以 `specs/025-workspace-client-sdk/tasks.md` 为准。Frontend Console 开发保持暂停且 `apps/console` 尚未存在；Issue #52 仍负责 trusted-publication 完整负向 Compose 验收。后续密钥轮换、跨副本 replay、生产治理、完整部署集成和 T020/T021 所需的 retention、shutdown 与 in-flight 策略仍需独立 Spec/ADR，不得把后续未决策略提前实现为 fallback。本次状态更新原因是 Spec 025 已形成可验证的应用调用边界，影响是 Go 应用可通过 `sdks/client-sdk` 使用已安装 Agent，但不得绕过 Gateway 或恢复 v3/直连兼容路径。
